			...
			for i in range(0, 4):
				if col1 and col5:
					if (ord(ramp[i][1:2]) == ord(fountain.name[1:2])) and ord(ramp[i][0:1]) <  ord(fountain.name[0:1]):
						OK = True
				elif (abs(ord(ramp[i][1:2]) - ord(fountain.name[1:2])) <= 1 and ord(ramp[i][0:1]) <  ord(fountain.name[0:1])):
					OK = True
	
			if not OK:
				housesOkay = False
		
		#check for rampts to get to houses in d3. d2/d4 houses will always hit this condition.
		OK = False
		if d3 and c3ledge:
			for i in range(0, 4):
